...
def grad(I):
    ...
    kernel = torch.tensor([[1 / 8, 1 / 8, 1 / 8], [1 / 8, -1, 1 / 8], [1 / 8, 1 / 8, 1 / 8]], dtype=torch.float32)
    kernel = kernel.unsqueeze(0).unsqueeze(0)  # 扩充维度，变成4维Tensor和输入的图像shape保持一致
    #保障所有的tensor都在同一个设备上（CPU或者GPU）
    if use_gpu:
        kernel = kernel.cuda()
    else:
        kernel = kernel
    ...
    grads = []
    for c in range(C):
        g = torch.nn.functional.conv2d(I[:, c:c + 1, :, :], kernel, stride=1, padding=1)
        grads.append(g)
    return torch.cat(grads, dim=1)  # concatenate along the channel dimension to get the final gradients tensor

#自定义L1范数实现
def L1(batchimg):
    L1_norm = torch.norm(batchimg, p=1, dim=[2, 3])
    return L1_norm

# 自定义联合梯度计算函数
def joint_grad(source1,source2,fused):
    grad_source1 = grad(source1)
...
def intensity(source,fused):
     return torch.mean(torch.square(source-fused))
