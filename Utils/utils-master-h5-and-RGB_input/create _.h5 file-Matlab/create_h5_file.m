% %%% 制作CT-MRI_114.h5和CT-MRI_22.h5
% clc;
% clear;
% close all;
% 
% %%% 预定义常量
% %savepath='F:\论文实验复现\杨老师\马佳义团队\2021\EMFusion-main\CT-MRI\CT-MRI_114.h5';
% savepath='F:\论文整理构思\构思实现\EMFusion3\CT-MRI_22.h5';
% %size_input=114;
% size_input=22;
% stride=19;
% %stride=30;    %步长决定取块的频率
...
% 
% % initialization  初始化
% data=zeros(size_input,size_input,2*img_ch,1); %256x256x2x1的全零矩阵,这四个参数分别控制WHCN
% 
% %两个源图像对应的文件夹(这里用自己找的10对)
% folder1='F:\论文整理构思\训练数据集\医学\Harvard-Medical-Image-Dataset-main\CT-MRI\CT';
% filepath1=dir(fullfile(folder1,Format));
% folder2='F:\论文整理构思\训练数据集\医学\Harvard-Medical-Image-Dataset-main\CT-MRI\MRI';
...
% 
% %%% 预定义常量
% savepath='F:\论文实验复现\杨老师\马佳义团队\2023\MURF-main\PET-MRI\shared_information_extraction\aligned_train.h5';
% size_input=256;
% stride=1;  %这里size_input和源图像大小一致，所以步长无关紧要(步长决定取块的频率)
% Format='*.png';
% img_ch=1; 
% ratio=1; 
% 
...
%%% 制作GFP-PCI_120.h5  !!!!!!!!!!注意，对于24位图（不管是真彩色还是假彩色）要RGB_to_gray转换之后来制作，不然会损失两个通道的信息
%%% ！！！！！！！！！制作出来里面的图像已经是除过255了
clc;
clear;
close all;
...
%%% 预定义常量
savepath='F:\论文实验复现\杨老师\刘羽\MATR-main\GFP-PCI_120.h5';
size_input=120;
stride=20;  %这里size_input和源图像大小一致，所以步长无关紧要(步长决定取块的频率)
Format='*.jpg';
img_ch=1; 
ratio=1; 
...
%两个源图像对应的文件夹(这里用自己找的10对)
folder1='F:\论文实验复现\杨老师\刘羽\MATR-main\source images\GFP-PC\GFP';
filepath1=dir(fullfile(folder1,Format));
folder2='F:\论文实验复现\杨老师\刘羽\MATR-main\source images\GFP-PC\PCI';
filepath2=dir(fullfile(folder2,Format));
...
for i=1:L
    %读取一对源图像
    img1=rgb2gray(imread(fullfile(folder1,filepath1(i).name)));
    % img2=imread(fullfile(folder2,filepath2(i).name));
    img2=imread(fullfile(folder2,[filepath1(i).name(1:7),'t',filepath1(i).name(9:12)]));
    disp([filepath1(i).name(1:7),'t',filepath1(i).name(9:12)])
